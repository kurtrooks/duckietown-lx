...
def delta_phi(ticks: int, resolution: int) -> float:
    dphi = 2.*np.pi*float(ticks)/resolution
    return dphi
...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    ### KR Code ###
    delta_phi_left = delta_phi(left_encoder_ticks,robot_spec["encoder_resolution"])
    delta_phi_right = delta_phi(right_encoder_ticks,robot_spec["encoder_resolution"])

    d_t,y,phi_t = pose_estimation(robot_spec["wheel_radius"],robot_spec["wheel_baseline"], 
                                grid_spec["d"],0.,grid_spec['phi'],delta_phi_left,delta_phi_right)
